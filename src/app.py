...
def add_personaje_favorito(people_id):
    ...
    if favorite_exists:
        return {"error": f"ya existe el personaje: {body_user_id, body_people_id}"}, 400
...
#Añade un nuevo planet favorita al usuario actual con el planet.id = planet_id.
...
def add_planeta_favorito(planet_id):
    ...
    if favorite_exists:
        return {"error": f"ya existe el planeta: {body_user_id, body_planet_id}"}, 400
    new_favorites = Favorite(user_id=body_user_id, planet_id=body_planet_id)
    db.session.add(new_favorites)

    try:
        db.session.commit()
        return jsonify({"msg": "Planeta Favorito creado"}), 201
    except Exception as error:
        db.session.rollback()
        return jsonify({"error": error})
...
def delete_favorite_people(people_id, user_id):
    ...
    if not favorite_delete:
        return {"error": "El personaje favorito no existe"}, 400
    db.session.delete(favorite_delete)
    try:
        db.session.commit()
        return {"msg": "el personaje favorito se elimino"}, 200
    except Exception as error:
        db.session.rollback()
        return jsonify({"error": error})
...
def delete_favorite_planet(planet_id, user_id):
    ...
    if not favorite_delete:
        return {"error": "El planeta favorito no existe"}, 400
    db.session.delete(favorite_delete)
    try:
        db.session.commit()
        return {"msg": "el planeta favorito se elimino"}, 200
    except Exception as error:
        db.session.rollback()
        return jsonify({"error": error})
